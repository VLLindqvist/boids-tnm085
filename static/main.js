...
class ThreeScene {
  constructor() {
    ...
    this.camera.position.z = controls.depth * controls.scaleY * 1.8;
  ...
  drawInitalScene() {
    ...
    const backgroundGeometry = new THREE.BoxGeometry(
      controls.width * 2 * controls.scaleX,
      controls.height * 2 * controls.scaleY,
      controls.depth * 2 * controls.scaleZ,
      10,
      10,
      10
    );
  ...
  render() {
    ...
    if (
      controls.scaleX != this.container.scale.x ||
    ...
    if (controls.play) {
      ...
      this.flock.boids.forEach((boid) => {
        ...
        // if (uuidToLookFor === boid.mesh.uuid) {
        //   console.log({ velocity: boid.velocity });
        // }
