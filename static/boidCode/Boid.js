...
class Boid {
  ...
  flaxa() {
    this.mesh.children[1].rotateZ(
      THREE.Math.degToRad(this.flaxStep * controls.speed)
    );
    this.mesh.children[2].rotateZ(
      THREE.Math.degToRad(-this.flaxStep * controls.speed)
    );
  ...
  createModel() {
    return new Promise((resolve) => {
      ...
      let boidMaterial = new THREE.MeshLambertMaterial({
  ...
  interact_with_flock(boids) {
    ...
    const xIn =
      Math.cos(-PI_HALF) * this.velocity.x -
      Math.sin(-PI_HALF) * this.velocity.y;
    const yIn =
      Math.sin(-PI_HALF) * this.velocity.x +
      Math.cos(-PI_HALF) * this.velocity.y;
    ...
    if (this.velocity.angleTo(newVelo) > maxRot) {
      ...
      newVelo.setComponent(
        0,
        Math.cos(maxRot) * this.velocity.x - Math.sin(maxRot) * this.velocity.y
      ); // Set new X
      newVelo.setComponent(
        1,
        Math.sin(maxRot) * this.velocity.x + Math.cos(maxRot) * this.velocity.y
      ); // Set new Y
    }
  ...
  avoidBorder(newVelo, maxRot) {
    ...
    sumVector.add(
      this.avoid(
        new THREE.Vector3(
          controls.width * controls.scaleX,
          this.position.y,
          this.position.z
        )
      )
    ...
    sumVector.add(
      this.avoid(
        new THREE.Vector3(
          this.position.x,
          controls.height * controls.scaleY,
          this.position.z
        )
      )
    );
    ...
    sumVector.add(
      this.avoid(
        new THREE.Vector3(
          this.position.x,
          this.position.y,
          controls.depth * controls.scaleZ
        )
      )
    );
    sumVector.add(
